      ...
      drawProgress() {
        draw.init({
          ...
          callback_move: (data) => {
            ...
            let rateX = parseInt((moveX / data.total.X) * 100) / 100;
            this.changeLine(rateX);
            addClass(this.$refs.hand, 'show');
            // 通知父组件修改props
            this.$emit('changeVoiceRate', rateX);
          },
          callback_up: () => {
            removeClass(this.$refs.hand, 'show');
          }
        });
      }
    }
